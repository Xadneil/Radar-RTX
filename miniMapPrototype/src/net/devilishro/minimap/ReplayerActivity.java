...
public class ReplayerActivity extends Activity {
	...
	private void next_post(){
		ArrayList<ContentValues> temp = AppState.recv_points(counter); // list of info from all players on the filed at a certain time
		...
		for(int i = 0; i < temp.size(); i++)
		{
			...
			if(i == 0)
				set_map(new LatLng(cur_pos.getAsDouble(ReplayDatabase.KEY_NAME2), cur_pos.getAsDouble(ReplayDatabase.KEY_NAME3)));
			id = (Integer) cur_pos.get(ReplayDatabase.KEY_NAME1);
			val = mark.get(id); //gets the marker
			if(val == null)
			{   //new player for the replay
				val = map.addMarker(new MarkerOptions().snippet(AppState.getNames().get(id))
						.position(pos));
			}
			move_marker(val, new LatLng(cur_pos.getAsDouble(ReplayDatabase.KEY_NAME2), cur_pos.getAsDouble(ReplayDatabase.KEY_NAME3)));
		}
